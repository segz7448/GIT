  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: spacing.lg }}>
      ...
      <View style={styles.detailsCard}>
        {!!user.company && <DetailRow icon="building" text={user.company} />}
        {!!user.location && <DetailRow icon="pin" text={user.location} />}
        {!!user.email && <DetailRow icon="mail" text={user.email} />}
        {!!user.blog && (
          <TouchableOpacity onPress={() => Linking.openURL(user.blog.startsWith('http') ? user.blog : `https://${user.blog}`)}>
            <DetailRow icon="link" text={user.blog} linkStyle />
          </TouchableOpacity>
        )}
      </View>
