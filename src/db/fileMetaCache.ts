...
export async function getCachedFileMeta(
  ...
  const row = await db.getFirstAsync<{
    ...
    cached_at: number;
  }>(`SELECT * FROM file_meta_cache WHERE meta_key = ?`, [key]);
  if (!row) return null;
  return {
...
async function storeFileMeta(
  owner: string,
  repo: string,
  branch: string | null | undefined,
  path: string,
  meta: Omit<FileMeta, 'cachedAt'>
): Promise<void> {
  const db = await withDb();
  const key = fileMetaKey(owner, repo, branch, path);
  await db.runAsync(
 ...
 */
export async function getOrFetchFileMeta(
  owner: string,
  repo: string,
  branch: string | null | undefined,
  path: string
  ...
  try {
    ...
    const meta = {
      lastModified: latest.commit?.committer?.date ? new Date(latest.commit.committer.date).getTime() : null,
