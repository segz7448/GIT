...
export async function listStashes(owner: string, repo: string): Promise<Stash[]> {
  const db = await withDb();
  const rows = await db.getAllAsync<RawStashRow>(
...
export async function getStash(id: string): Promise<Stash | null> {
  const db = await withDb();
  const row = await db.getFirstAsync<RawStashRow>(`SELECT * FROM stashes WHERE id = ?`, [id]);
...
function mapRow(row: RawStashRow): Stash {
  let files: unknown[] = [];
  try {
