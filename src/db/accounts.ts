...
export async function addAccount(username: string, token: string, label?: string | null): Promise<string> {
...
export async function listAccounts(): Promise<Account[]> {
  const db = await withDb();
  const rows = await db.getAllAsync<RawAccountRow>(`SELECT * FROM accounts ORDER BY last_used_at DESC`);
  return rows.map(mapRow);
}

export async function getAccountToken(accountId: string): Promise<string | null> {
  const db = await withDb();
  const row = await db.getFirstAsync<{ token_key: string }>(`SELECT token_key FROM accounts WHERE id = ?`, [
    accountId,
  ]);
  if (!row) return null;
...
export async function updateAccountLabel(accountId: string, label: string | null): Promise<void> {
  const db = await withDb();
...
function mapRow(row: RawAccountRow): Account {
  return {
    id: row.id,
