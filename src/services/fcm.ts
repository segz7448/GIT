...
// notifications.ts). It does NOT send anything on its own - something
...
async function persistToken(token: string | null | undefined): Promise<void> {
  if (!token) return;
...
export function registerBackgroundHandler(): void {
  messaging().setBackgroundMessageHandler(async (_remoteMessage: FirebaseMessagingTypes.RemoteMessage) => {
