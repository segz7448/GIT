...
function withWidgetNativeFiles(config) {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      // res/layout, res/xml, and res/drawable resources
      ...
      const destDrawableDir = path.join(projectRoot, 'app/src/main/res/drawable');
      ...
      const drawableSourceDir = path.join(RESOURCES_DIR, 'drawable');
      if (fs.existsSync(drawableSourceDir)) {
        for (const file of fs.readdirSync(drawableSourceDir)) {
          fs.copyFileSync(path.join(drawableSourceDir, file), path.join(destDrawableDir, file));
        }
      }
